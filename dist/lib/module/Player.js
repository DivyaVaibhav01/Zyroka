...
class Player {
    ...
    async setVolume(volume) {
        if (Number.isNaN(volume))
            throw new RangeError('<Zyroka> Volume level must be a number.');
        await this.shoukaku.setGlobalVolume(volume);
        this.volume = volume;
    }
    async pause(pause = true) {
        if (typeof pause !== 'boolean')
            throw new RangeError('<Zyroka> Pause function must be pass with boolean value.');
        this.paused = pause;
        this.playing = !pause;
    ...
    setTextChannel(textId) {
        if (typeof textId !== 'string')
            throw new RangeError('<Zyroka> textId must be a string.');
        this.textId = textId;
    }
    setVoiceChannel(voiceId) {
        if (typeof voiceId !== 'string')
            throw new RangeError('<Zyroka> voiceId must be a string.');
