...
export class Player {
    ...
    public async play() {
        try {
            ...
            await this.shoukaku.playTrack({ track: { encoded: this.queue.current?.encoded } });
    ...
    public async setVolume(volume: number) {
        if (Number.isNaN(volume)) throw new RangeError('<Zyroka> Volume level must be a number.');
        await this.shoukaku.setGlobalVolume(volume);
        this.volume = volume;
    }

    public async pause(pause = true) {
        if (typeof pause !== 'boolean') throw new RangeError('<Zyroka> Pause function must be pass with boolean value.');
        this.paused = pause;
        this.playing = !pause;
    ...
    public setTextChannel(textId: string) {
        if (typeof textId !== 'string') throw new RangeError('<Zyroka> textId must be a string.');
        this.textId = textId;
    }

    public setVoiceChannel(voiceId: string) {
        if (typeof voiceId !== 'string') throw new RangeError('<Zyroka> voiceId must be a string.');
